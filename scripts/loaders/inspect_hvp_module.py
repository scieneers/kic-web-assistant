"""
Test-Script: Inspiziert HVP-Modul (Kurs 106, Modul 7159)

Ruft core_course_get_contents für Kurs 106 ab und zeigt das JSON
für das HVP-Modul 7159 an.
"""
...
def main():
    ...
    # Ausgabe
    print("=" * 80)
    print(f"MODUL {MODULE_ID} - JSON-STRUKTUR")
    print("=" * 80)
    ...
    if target_module.get('contents'):
        print(f"\nContents ({len(target_module['contents'])} Einträge):")
        for i, content in enumerate(target_module['contents'], start=1):
            print(f"  [{i}] {content.get('filename')} ({content.get('type')})")
